(() => {
  ...
  const config = Object.assign({
    ...
    loginCaptchaEnabled: '1', authCaptchaEnabled: '1', frontendVersion: '1.2.29'
  }, window.XBOARD_THEME || {});
  ...
  async function renderTickets(id) {
    ...
    try {
      ...
      const closedCount = closedTicketCount(state.tickets);
      ...
      app.innerHTML = shell(`${pageHead(t('nav_tickets'), t('nav_tickets'), t('tickets_title'), `<button class="btn btn-ticket-cta" data-action="new-ticket">${icon('ticket')}<span>${t('new_ticket')}</span></button>`)}
        <div class="grid grid-3" style="margin-bottom:24px">
          ...
          <div data-ticket-stat="closed">${statCard('bell', closedCount, tx('ticket_closed_count'), 'bg-gradient-warning')}</div>
        </div>
        <section class="card">${rows ? `<div class="table-wrap"><table class="table ticket-table"><thead><tr><th>${t('ticket_subject')}</th><th>${t('status')}</th><th>${ticketReplyStatusLabel()}</th><th>${t('create_time')}</th><th>${tx('ticket_last_reply_time')}</th><th>${t('action')}</th></tr></thead><tbody>${rows}</tbody></table></div>` : empty(tx('tickets_none_title'), tx('tickets_none_sub'))}</section>`, t('nav_tickets'));
    } catch (error) { renderError(error); }
  }
  ...
  function refreshTicketsPageUnreadUi() {
    ...
    table.querySelectorAll('tr[data-ticket-id]').forEach(row => {
      ...
      if (statusCell && item) {
        statusCell.innerHTML = `<span class="status ${isTicketClosed(item) ? 'warning' : 'success'}">${isTicketClosed(item) ? t('ticket_closed') : t('ticket_open')}</span>`;
      }
    ...
    const closedNode = app.querySelector('[data-ticket-stat="closed"] .meta strong');
    if (closedNode) closedNode.textContent = String(closedTicketCount(state.tickets));
  }
  ...
  document.addEventListener('click', async event => {
    ...
    if (action === 'open-mobile-menu') {
      ...
      await closeTicketById(target.dataset.id);
